...
stack_t *stack_construct(stack_t *self, size_t capacity) {
    ...
    #else
    self->data = (stack_elem_t *)malloc(capacity * sizeof(stack_elem_t));
    #endif
...
int stack_resize(stack_t *self, size_t capacity) {
    ...
    #if STACK_VALIDATION_LEVEL >= 1
    stack_elem_t *newData = (stack_elem_t *)realloc(self->data, capacity * sizeof(stack_elem_t) + 2 * sizeof(canary_t));
    #else
...
stack_validity_e stack_validate(stack_t *self) {
    ...
    #if STACK_VALIDATION_LEVEL >= 1
    if ((isPointerValid(stack_rightDataCanary(self)) && *stack_leftDataCanary(self) != CANARY) || \
